...
L'adozione \emph{enterprise}, tuttavia, ha seguito un percorso diverso e più incerto, 
...
Queste tre criticità delimitano il punto di partenza del percorso argomentativo della tesi, che dal contesto generale dell'adozione enterprise dell'AI generativa procede fino ai contributi specifici qui proposti (Fig.~\ref{fig:imbuto_narrativo}).
...
\begin{enumerate}
  ...
  le variazioni di performance. La valutazione si estende inoltre alla qualità dei
  sub-agenti, alla catalogazione delle tecniche di \emph{prompt engineering} applicate
  ai dati aziendali reali dei singoli agenti e all'analisi quantitativa del trade-off
  costo--qualità realizzato dal routing per complessità.
  Capitolo~\ref{chap:orion-valutazione}.
\end{enumerate}

I tre contributi appena elencati sono sintetizzati nella Fig.~\ref{fig:contributi_sintesi}.
...
\begin{itemize}
  ...
  \item Soprattutto, questa tesi \textbf{non sostiene che l'architettura proposta sia universalmente la risposta giusta}. 
  Mostra che lo è \emph{in questo contesto}, alla luce degli specifici vincoli esposti e in vigore: le possibilità offerte da questa
  tecnologia evolvono a velocità notevole e il contesto stesso potrebbe mutare in tempi brevi.
...
Il \textbf{Capitolo~\ref{chap:agenti}} presenta la suite di agenti verticali,
dai principi guida (principi etici, governance, agnosticità, sovranità del dato,
modularità verticale, automazione dei rilasci e controllo degli accessi)
alle fondazioni trasversali (stack tecnologico, organizzazione multi-repository,
infrastruttura Terraform, pipeline CI/CD, identità e catena di sicurezza),
fino ai dettagli realizzativi dei quattro agenti.

Il \textbf{Capitolo~\ref{chap:orion}} descrive l'orchestratore Orion:
motivazione e posizionamento rispetto alla letteratura,
routing step con output strutturato e logica di confidence e clarification,
agent proxy, gestione della storia conversazionale,
frontend con badge trasparente, admin panel,
infrastruttura e schema del database.

Il \textbf{Capitolo~\ref{chap:orion-valutazione}} riporta i passi di valutazione sperimentale 
eseguiti su più piani:
il routing dell'orchestratore (domande di ricerca, gold standard, metriche dichiarate a priori,
anatomia del prompt e tecniche di prompt engineering applicate ai dati aziendali, risultati e iterazione del prompt),
la qualità dei sub-agenti (execution accuracy per gli agenti Text-to-SQL e faithfulness per il RAG),
l'ottimizzazione costo--qualità del routing per complessità e una valutazione economica a ordini di grandezza.
