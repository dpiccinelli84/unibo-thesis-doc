...
\begin{description}
  ...
  \item[Rispetto dei principi etici e cooperativi]
    ...
    Offrire un pacchetto di assistenti interni mitiga questo rischio e previene insieme il
    ricorso a strumenti AI esterni non autorizzati (\emph{Shadow AI}), riducendo i rischi per
    la sicurezza del dato. I prompt di sistema, stabiliti collegialmente fra gli uffici interni,
    integrano direttive esplicite che allineano ogni agente all'identità e ai valori
    dell'organizzazione, in conformità con i requisiti di trasparenza e \emph{human oversight}
  ...
  \item[Governance facilitata e efficace]
    Le configurazioni del prompt di sistema (la ``personalità'' dell'agente) e dei controlli su
    utilizzi impropri e dati personali devono essere gestibili da una dashboard apposita, usabile
    anche dagli uffici preposti privi di competenza tecnica, che consenta inoltre di monitorare
    --- nel rispetto della privacy --- gli eventi segnalati come ``pericolosi''. Tutto il codice
    sorgente risiede nel repository Git dell'organizzazione ed è proprietà della Cooperativa.

  \item[Implementazione agnostica e sostenibile]
    Le soluzioni AI ``out of the box'' dei fornitori comportano spesso \emph{vendor lock-in}
    (difficile portabilità verso altre infrastrutture) e costi fissi di sottoscrizione anche con
    utilizzo scarso. La soluzione realizzata deve invece essere portabile verso qualunque
    architettura e consentire un controllo dinamico dei costi: un budget di utilizzo (``token'')
...
L'ecosistema è accessibile da un unico ``portale'', conseguenza diretta della modularità
verticale: i nuovi agenti si innestano nel \emph{framework} esistente e le dashboard di
configurazione (``admin panel'') sono radunate in un portale dedicato.
...
cloud attivo in Coop Alleanza (le risorse on-premises non consentono una realizzazione
performante e sostenibile di progetti di AI generativa).
...
\begin{enumerate}
  \item \textbf{Sanitizzazione del prompt.} Prima dell'invio all'LLM, il testo passa per il template
        di Google Cloud Model Armor~\cite{gcp_modelarmor}, che combina un classificatore
