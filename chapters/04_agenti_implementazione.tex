...
tali scelte sono state effettivamente realizzate. La trattazione è organizzata in
due movimenti: dapprima si descrivono gli elementi trasversali alla suite (stack
...
\begin{itemize}
  ...
  \item \textbf{Database e Data Warehouse.} Cloud SQL per
        ...
        dataset alimentati in modo continuo da pipeline di replicazione
        esterne al perimetro della suite.
  ...
  \item \textbf{Orchestrazione di agenti.} Per gli agenti più sofisticati
        (Vera ed AllyCare) viene utilizzato il \emph{Google Agent
        ...
        \emph{function calling}. Per CooPolicy e per la variante Gemini
        di SysAid si è preferita l'invocazione diretta dell'SDK
        \texttt{vertexai.generative\_models}, dove un controllo
        granulare sul ciclo di chiamate vale di più dell'astrazione
        offerta da ADK. La scelta evita di assumere una dipendenza
        comune (LangChain~\cite{langchain_docs} o
        LlamaIndex~\cite{llamaindex_docs}) che vincolerebbe l'evoluzione
        a una sola comunità.
...
contiene il proprio \texttt{Dockerfile}, le proprie dipendenze, il proprio
file di descrizione della pipeline (\texttt{cloudbuild.yaml}) e, dove
necessario, una piccola suite di test.
...
Vera è il backend più articolato della suite e l'unico che nasce da una
codebase pre-esistente, gradualmente refattorizzata in moduli verticali
(\texttt{api/}, \texttt{services/}, \texttt{adk/}, \texttt{models/},
\texttt{schemas/}, \texttt{core/}). Le scelte realizzative significative
...
La selezione del modello è governata dal modulo \texttt{model\_router.py} con
una logica a due livelli. Il primo livello deriva dal badge: Bronze è limitato
a Gemini~2.5 Flash, Gold ha accesso esclusivo a Gemini~2.5 Pro, Silver è in
\emph{tier} intermedio. Il secondo livello, attivo solo per Silver, valuta
...
recupero, ad ancorare la generazione esclusivamente al materiale recuperato e
a rispettare le ACL aziendali sui documenti.
...
classi di documenti hanno regole di accesso o di interpretazione
...
corpus disponibile e due regole-guida specifiche per il dominio coop
(domande su indennità di livello impongono il corpus contratti, domande
su nomi propri o organigrammi impongono il corpus governance), che
correggono ex ante errori di classificazione osservati durante la fase di
\emph{tuning}.
...
quali chunk siano effettivamente utili. Il rationale è che un modello in
...
pannello laterale del frontend.
...
europeo e soddisfano il requisito di sovranità del dato, ma l'asimmetria
ha conseguenze pratiche (latenza media leggermente più alta per Sonnet
da Bologna, configurazioni Terraform separate) che i deploy futuri di
nuovi agenti dovranno tenere in considerazione.
