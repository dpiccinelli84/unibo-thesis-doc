...
inizialmente sono descritti gli elementi trasversali del progetto (stack
...
\begin{itemize}
  ...
  \item \textbf{Large Language Models.} L'implementazione poggia su due
        ...
        dei due fornitori in SysAid (Sez.~\ref{sec:agenti-sysaid-impl})
        concretizza un caso di agnosticità come descritto nel
...
\emph{Implementazione sostenibile} del
...
di modularità verticale e accesso centralizzato descritti nel
Capitolo~\ref{chap:agenti-architettura}.
...
Vera è l'agente più articolato; le scelte realizzative significative
ruotano attorno a tre problemi: \emph{routing} del modello,
\emph{Web grounding} con citazioni inline, gestione dei documenti
...
descritto nel Cap.~\ref{chap:orion-implementazione}, implementa la stessa
...
valori dell'organizzazione, descritte fra i principi guida del
Cap.~\ref{chap:agenti-architettura}) come misura di protezione a livello
di prompt.
...
AllyCare è la realizzazione del paradigma Text-to-SQL (Sez.~\ref{subsec:bg-text2sql}) su BigQuery per
l'analisi dei sondaggi NPS. L'implementazione
espone al modello tre \emph{tool} di
interazione con i dati e affida all'\emph{agentic loop} di ADK la
gestione del ciclo ReAct (Sez.~\ref{sec:bg-multiagent}).
...
SysAid implementa l'architettura a doppio code path descritta nella Sez.~\ref{subsec:agenti-sysaid-confronto}: un unico backend ospita due code path
parallele, alimentate rispettivamente da Gemini~2.5 Flash su Vertex AI~\cite{gcp_vertexai}
e da Claude Sonnet~4.6 per il tramite di \texttt{AnthropicVertex}~\cite{anthropic_docs}.
La selezione avviene sulla base della complessità stimata della query, valutata dal modello Flash.
...
Cap.~\ref{chap:orion-implementazione}, replica le stesse funzionalità
