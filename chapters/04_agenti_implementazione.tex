...
bitbucket/c30-ai-agents/
  vera-backend/         # Repo Vera Backend (FastAPI + ADK + Gemini)
  vera-frontend/        # Repo Vera Frontend
  docs-backend/         # Repo CooPolicy Backend (FastAPI + ADK + RAG)
  nps-backend/          # Repo AllyCare Backend (FastAPI + ADK + BigQuery)
  nps-frontend/         # Repo AllyCare Frontend
  sysaid-backend/       # Repo SysAid Gemini Backend (FastAPI + ADK)
  sysaid-sonnet-backend/# Repo SysAid Claude Backend (FastAPI + ADK)
  sysaid-frontend/      # Repo SysAid Frontend
  portal-frontend/      # Repo Portale unico di accesso (MSAL)
  admin-backend/        # Repo Pannello amministrativo Backend
  admin-frontend/       # Repo Pannello amministrativo Frontend
  iac-infrastructure/   # Repo Terraform per infrastruttura
  ai-dev-documents/     # Repo Documentazione architetturale
  \end{lstlisting}
  \caption[Struttura dei repository]{Elenco dei repository principali della suite. Ogni servizio è un progetto Git autonomo con proprio ciclo di CI/CD e branch per ambiente.}
...
% [FIGURA 4.2] Pipeline CI/CD (Cloud Build -> Artifact Registry -> Cloud Run).
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.9\textwidth]{fig04_cicd_pipeline.pdf}
%     \caption[Pipeline CI/CD con Cloud Build]{Schema della pipeline di Continuous Integration e Continuous Deployment per gli agenti della suite. Ad ogni \emph{push} sui branch designati, Cloud Build compila il container e lo rilascia su Cloud Run.}
...
% [FIGURA 4.3] Catena di sicurezza pre-generazione (DLP + Model Armor).
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.9\textwidth]{fig04_security_chain.pdf}
%     \caption[Catena di sicurezza pre e post generazione]{La catena di sicurezza che ogni richiesta attraversa prima di raggiungere il modello e prima di essere restituita all'utente, integrata con Model Armor e Sensitive Data Protection.}
...
La selezione del modello è governata dinamicamente dal modulo \texttt{model\_router.py}: esso valuta
euristicamente la complessità della richiesta sommando una serie di indicatori
(lunghezza del prompt, presenza di parole-chiave riconducibili ad analisi articolate, struttura della domanda) 
e instrada costantemente verso il modello più opportuno (es. da Flash a Pro o Sonnet) all'aumentare 
della complessità. A differenza di implementazioni più rigide, il livello di abbonamento o "badge" (Bronze, Silver, Gold) 
attribuito all'utente non interferisce mai sulla scelta del modello, ma agisce unicamente
come limite al \emph{budget di token} consumabili mensilmente.

Questa logica realizza in forma robusta, e per uso interno al singolo
...
piccolo ed economico è sempre preferito quando è sufficiente, mentre un modello più grande è impiegato
solo quando l'effettiva complessità della query lo giustifica, garantendo sostenibilità economica e massima qualità per tutti gli utenti, a prescindere dal loro budget.
...
\subsection{Variante Gemini: integrazione ADK nativa}
\label{subsec:sysaid-gemini}

La variante \texttt{sysaid-backend} utilizza appieno il \emph{Google Agent Development Kit} (ADK)
...
identiche. L'adozione di ADK semplifica enormemente il codice di base, permettendo di 
concentrarsi sulla logica di business.

\subsection{Variante Sonnet: astrazione tramite ADK e adapter}
\label{subsec:sysaid-sonnet}

La variante \texttt{sysaid-sonnet-backend} accede a Claude Sonnet
sempre sfruttando l'architettura ADK, ma instradando le chiamate verso \texttt{AnthropicVertex}, 
l'integrazione che permette di chiamare i modelli Anthropic mantenendo l'autenticazione 
...
l'implementazione fa uso di un \emph{thin adapter} all'interno del framework ADK 
per uniformare la semantica.

\paragraph{Definizione dei tool.} I tool rimangono concettualmente identici e 
vengono traslati nel formato JSON~Schema richiesto da Claude. Il sotto-insieme \texttt{SYSAID\_TOOLS\_REPORT}
omette \texttt{generate\_chart} ed è impiegato dall'endpoint di report
giornalieri (cfr. canale WhatsApp), che non può renderizzare immagini.

\paragraph{Ciclo e \texttt{stop\_reason}.} L'agentic loop di ADK si adatta per interpretare 
...
e lo stesso approccio strutturato visto nella variante Gemini. Quando il tool loop termina, 
...
dell'orchestratore.
à perciò necessario per ospitare la telemetria
dell'orchestratore.
ta da Vera, replicata
con piccole variazioni dagli altri agenti) e i log strutturati di
\texttt{security\_logger} costituiscono il fondamento dello stesso
\emph{audit trail} che, una volta arricchito con il record di
\texttt{orion\_routing\_logs} (Cap.~\ref{chap:orion-implementazione}),
...
servizio sarà perciò necessario per ospitare la telemetria
dell'orchestratore.
security\_logger} costituiscono il fondamento dello stesso
\emph{audit trail} che, una volta arricchito con il record di
\texttt{orion\_routing\_logs} (Cap.~\ref{chap:orion-implementazione}),
abiliterà la dashboard \emph{RouterMetrics} per la valutazione
sperimentale della Sez.~\ref{chap:orion-valutazione}. Nessun nuovo
