...
disegno obiettivo del progetto, mostrato nella sua forma finale in
Fig.~\ref{fig:architettura_orion_v2}: ai repository della suite
...
formalizza il routing come un \emph{cascade} sequenziale (Sez.~\ref{sec:bg-routing}).
...
\paragraph{Orion vs RouteLLM.} RouteLLM~\cite{ong2024routellm}, con il suo router appreso (Sez.~\ref{sec:bg-routing}), decide tra un modello forte e uno debole.
...
sistema secondo una logica a soglia configurabile (Fig.~\ref{fig:routing_decision}).
...
più incerto quando sbaglia e più sicuro quando ha ragione (Sez.~\ref{sec:bg-confidence}) --- non è
banale e peggiora proprio sui domini specifici non visti in addestramento. Il Capitolo~\ref{chap:orion-valutazione} ne verifica empiricamente la tenuta sul dominio del progetto.
...
\texttt{forward}. La tabella degli endpoint (Tab.~\ref{tab:agent-endpoints}) è configurata tramite variabili
...
\paragraph{Gestione degli errori e fallback.} Se il sub-agente risponde con un codice di errore (4xx,
5xx) o supera il timeout di 120~s, il proxy restituisce un messaggio leggibile con il flag
\texttt{error = true}; il frontend lo intercetta e, in alternativa, il flusso può ricadere sulla
pipeline interna (\texttt{agent = "self"}).

\paragraph{Streaming bufferizzato.} Il proxy attende la risposta completa del sub-agente prima di
inoltrarla via SSE: la scelta semplifica la normalizzazione al costo di un \emph{time-to-first-token}
pari alla latenza dell'intero sub-agente; lo streaming \emph{pass-through} è una possibile evolutiva
(Tab.~\ref{tab:orion-decisioni}).

La Fig.~\ref{fig:orion_sequence} ricostruisce l'intero turno come sequence diagram, dalla richiesta utente al routing fino all'invocazione del sub-agente e alla normalizzazione della risposta.
...
Orion mantiene una \emph{storia unica} per conversazione: tutti i turni, qualunque agente li abbia
serviti, risiedono nella stessa conversazione, così che l'utente possa cambiare argomento (e agente)
senza discontinuità. Il trade-off è che il sub-agente riceve l'intera storia, inclusi i turni di altri
agenti: per mitigare la confusione la storia è troncata agli ultimi sei turni (configurabile) e i
contenuti non pertinenti (grafici base64, citazioni) sono sostituiti da un riassunto sintetico. Anche
il router riceve la storia, così che un follow-up come ``E quanti ne sono stati chiusi?'' venga
instradato a SysAid pur senza parole chiave esplicite.
...
La Fig.~\ref{fig:orion_switch} mostra il badge in azione: in un'unica conversazione due richieste consecutive sono instradate ad AllyCare e a SysAid, senza che l'utente debba cambiare strumento.
...
La Fig.~\ref{fig:orion_coopolicy} mostra una richiesta su una policy aziendale instradata a CooPolicy attraverso Orion: le citazioni numerate ai documenti sorgente sono preservate dal proxy e rese cliccabili dal frontend.
...
con tabelle distinte identificate dal prefisso \texttt{orion\_}; lo schema relazionale completo è riportato in Fig.~\ref{fig:orion_er}. Le tabelle principali sono:
